...
import static com.smile.wechat.R.id.ivHeader;
...
//                fields.put(TeamFieldEnum.Name, "群聊(" + accounts.size() + 1 + ")");
                    NimTeamSDK.createTeam(fields, TeamTypeEnum.Normal, accounts, new RequestCallback<CreateTeamResult>() {
                        @Override
                        public void onSuccess(CreateTeamResult param) {
                            hideWaitingDialog();
                            //跳转到SessionActivity
                            Intent intent = new Intent(TeamCheatCreateActvitiy.this, SessionActivity.class);
                            intent.putExtra(SessionActivity.SESSION_ACCOUNT, param.getTeam().getId());
